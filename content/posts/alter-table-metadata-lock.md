...
> Lưu ý: Bài viết này áp dụng với MySQL 8, vậy nên không chắc mọi thứ vấn sẽ đúng với MySQL 5.7 trở xuống.
...
Câu query này lâu một cách bất thường, khi chạy lệnh `SHOW FULL PROCESSLIST`, thì có một loạt câu query bị trạng thái `Waiting for table metadata lock`, trong đó có cả câu query ở trên luôn:
...
Tôi để câu query này 15 phút, nhưng càng ngày càng có nhiều query bị block lại. Tức là không chỉ mình bảng mà tôi đang thao tác bị lock, mà tất cả các query tới các bảng con có foreign key trỏ tới bảng này cũng bị lock theo. Điều này rất không ổn với một hệ thống đang chạy production, nhất là khi bảng con đang bị liên luỵ theo bảng tối quan trọng, được sử dụng rất nhiều trong hệ thống. Không còn cách nào khác, tôi phải ngừng câu query `ALTER TABLE` ở trên và tìm giải pháp khác.
...
Online DDL là một tính năng giúp thay đổi cấu trúc bảng trong khi đang có tác vụ trong bảng đó. Theo như [document MySQL này](https://dev.mysql.com/doc/refman/8.4/en/innodb-online-ddl-operations.html#online-ddl-table-operations), `ALTER TABLE` có thể chạy xong rất nhanh mà không bị lock gì khi ta set `ALGORITHM=INPLACE, LOCK=NONE` trong câu query:

```sql
ALTER TABLE `some_table` CONVERT TO CHARACTER SET `utf8` COLLATE `utf8_unicode_ci`, ALGORITHM=INPLACE, LOCK=NONE
```

ngay lập tức có lỗi sau đây xảy ra:
...
Thực tế còn một phương án cuối là restart lại MySQL, nhưng việc restart MySQL trong môi trường production là rất không nên (thực tế tôi cũng không có quyền restart RDS), nên chúng ta đào sâu kĩ hơn vấn đề. 
...
Vậy ta có sơ đồ đơn giản sau để mô tả vấn đề gặp phải khi `ALTER TABLE`:
...
Như ở bảng trên, ta thấy có rất nhiều các transaction đang chạy. Nếu một trong các transaction này đã/đang can thiệp vào bảng cha đang cần `ALTER TABLE` hoặc một trong các bảng con của nó, các query về sau mà liên quan tới bảng cha và các bảng con trong lúc đang `ALTER TABLE` bảng cha sẽ bị block lại. Vậy để cho câu query `ALTER TABLE` có thể chạy được, ta phải kết thúc transaction đang bị block.

> Tham chiếu cột `trx_mysql_thread_id` ở bảng `innodb_trx` với cột `id` trong `PROCESSLIST`, ta thấy tất cả các ID ở trong `trx_mysql_thread_id` tồn tại trong `PROCESSLIST`, điều đó củng cố luận điểm "transaction tồn tại không có nghĩa là phải có query xảy ra trong nó"
...
Hiện tại tôi chưa có cách nào hay hơn việc kill session đang chứa transaction đó

Để kill một session trong MySQL, ta chạy lệnh với `<session_id>` chính là `trx_mysql_thread_id`:
...
Không có gợi ý nào cho bạn để xem nên kill session nào, vì chẳng có thông tin gì thêm về các session. Để an toàn bạn có thể kill từng session một rồi chạy thử `ALTER TABLE` xem có chạy không, hoặc là kill tất cả các session và chạy một lần cho xong.

Cuối cùng tôi đã có thể chạy câu lệnh `ALTER TABLE` một cách thoải mái mà không cần phải đợi quá lâu.

# Ngăn ngừa

Như ở trên đã nêu, lý do xảy ra việc `Waiting for table metadata lock` là do transaction chưa được kết thúc khi không sử dụng. Việc để các transaction hanging còn gây tiêu tốn tài nguyên của MySQL, ví dụ như chứa rollback log của các câu query, chứ không chỉ block các DDL query (vốn hiếm khi thực hiện). Vậy để tránh việc này xảy ra, ta cần phải `ROLLBACK` hoặc `COMMIT` sau mỗi lần query DB. Dưới đây là 2 cách để quản lý transaction:
...
- StackOverflow
    * [MySQL 5.6 - table locks even when ALGORITHM=inplace is used](https://stackoverflow.com/questions/54667071/mysql-5-6-table-locks-even-when-algorithm-inplace-is-used)